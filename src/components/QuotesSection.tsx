...
const QuotesSection = () => {
  ...
  return <section className="py-10 px-4 sm:px-6 bg-gradient-to-r from-kb-mint/10 to-kb-blue/10">
      <div className="container mx-auto max-w-4xl">
        <div className="flex justify-center">
          <div className="w-full max-w-4xl text-center px-6 py-10 rounded-2xl bg-white/80 backdrop-blur-sm shadow-sm border border-kb-mint/20">
            <div className="relative h-44 md:h-32 lg:h-32 xl:h-32 flex items-center justify-center overflow-hidden">
            ...
            <div className="flex justify-center items-center space-x-2 mt-2">
              {quotes.map((_, index) => <span key={index} className={`block w-2 h-2 rounded-full transition-colors duration-300 ${currentQuoteIndex === index ? 'bg-primary' : 'bg-kb-mint/30'}`}></span>)}
            </div>
          </div>
        </div>
      </div>
